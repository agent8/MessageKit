...
open class ScrollToBottomButtonView: UIView {
    ...
    lazy var button: UIButton = {
        ...
        button.backgroundColor = EdoColor.cardBackground
        button.setImage(EdoTintImage("arrow-down-icon"), for: .normal)
        button.imageView?.tintColor = EdoColor.gray
        button.layer.borderWidth = 0.5
        button.layer.borderColor = EdoColor.gray.cgColor
        return button
    }()
    ...
    lazy var unreadBadgeCount: UIButton = {
        let button = RoundButton(type: .custom)
        button.backgroundColor = EdoColor.brandBlue
        button.setTitle("", for: .normal)
        button.setTitleColor(EdoColor.white, for: .normal)
